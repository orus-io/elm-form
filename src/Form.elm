...
type Builder validate view model customError sharedMsg output stackModel stackMsg topStackMsg
    = Builder
        ...
        , validate : String -> model -> Data.Model customError -> validate
...
init { validate, view } =
    Builder
        { load = []
        , validate = \_ model _ -> validate model
...
field :
    String
    -> FieldDef output a
    -> Builder (Validation customError a a -> validate) (FieldViewState customError a topStackMsg -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
    ...
    Builder
        ...
        , validate =
            \path model formData ->
                let
                    fieldState =
                        Data.getFieldAs fromField (FieldStack.path path name) formData
                in
                validate path model formData <| Validation.field fieldState.path fieldState.value
        , view =
...
fieldWithState :
    ...
    -> Builder (Validation customError a a -> validate) (FieldComponentViewState customError a topStackMsg componentModel componentMsg -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
...
list :
    ...
    -> Maybe (output -> List a)
    -> FieldDef output a
    -> Builder (Validation customError Never (List a) -> validate) (FieldListViewState customError a topStackMsg -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
    -> Builder validate view model customError sharedMsg output stackModel stackMsg topStackMsg
list name fieldlistload (FieldDef _ toField fromField) (Builder { validate, view, load, stack }) =
    Builder
        ...
        , validate =
            \path model formData ->
                Data.getListIndexes (FieldStack.path path name) formData
                    |> List.map
                        (\i ->
                            let
                                fieldState =
                                    Data.getFieldAs fromField (FieldStack.itempath path name i) formData
                            in
                            Validation.field fieldState.path fieldState.value
                        )
                    |> Validation.list
                    |> validate path model formData
        , view =
            \path toStackMsg model state ->
                let
                    fieldStates =
...
listWithState :
    ...
    -> FieldDef output a
    -> (FieldDef output a -> FieldStack.FieldComponent customError a componentModel sharedMsg componentMsg)
    -> Builder (Validation customError Never (List a) -> validate) (FieldComponentListViewState customError a topStackMsg componentModel componentMsg -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
    -> Builder validate view model customError sharedMsg output ( List componentModel, stackModel ) (FieldStack.Msg ( Int, componentMsg ) stackMsg) topStackMsg
...
group :
    String
    -> (output -> groupOutput)
    -> Builder (Validation customError groupOutput groupOutput) groupView model customError sharedMsg groupOutput groupStackModel groupStackMsg topStackMsg
    -> Builder (Validation customError groupOutput groupOutput -> validate) (groupView -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
    -> Builder validate view model customError sharedMsg output ( groupStackModel, stackModel ) (FieldStack.Msg groupStackMsg stackMsg) topStackMsg
group name getGroupData (Builder groupBuilder) (Builder builder) =
    Builder
        ...
        , validate =
            \path model state ->
                builder.validate path model state <|
                    groupBuilder.validate (FieldStack.path path name) model state
...
groupList :
    ...
    -> Builder (Validation customError Never groupOutput) groupView model customError sharedMsg groupOutput groupStackModel groupStackMsg topStackMsg
    -> Builder (Validation customError Never (List groupOutput) -> validate) (GroupListView groupView topStackMsg -> view) model customError sharedMsg output stackModel stackMsg topStackMsg
    -> Builder validate view model customError sharedMsg output ( List groupStackModel, stackModel ) (FieldStack.Msg ( Int, groupStackMsg ) stackMsg) topStackMsg
        ...
        , validate =
            \path model formData ->
                Data.getListIndexes (FieldStack.path path name) formData
                    |> List.map
                        (\i ->
                            groupBuilder.validate (FieldStack.itempath path name i) model formData
                        )
                    |> Validation.list
                    |> builder.validate path model formData
        , view =
            \path toStackMsg model state ->
                let
                    groupViewList =
...
finalize :
    Builder (Validation customError Never output) view model customError sharedMsg output stackModel stackMsg stackMsg
        ...
        \model msg state ->
            case msg of
                DataMsg formMsg ->
                    let
                        ( newData, newOutput ) =
                            Data.update (validation model) formMsg state.formData
                    in
                    ( { state
                        | formData = newData
